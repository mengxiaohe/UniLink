...
static I2C_HandleTypeDef *eeprom_hi2c; // I2C句柄指针
...
// 写入数据（自动处理页边界）
HAL_StatusTypeDef EEPROM_Write(uint16_t memAddr, uint8_t *data, const uint16_t size) {
    uint16_t bytesWritten = 0;
    while (bytesWritten < size) {
        // 计算当前页剩余空间
        const uint16_t pageOffset = memAddr % EEPROM_PAGE_SIZE;
        const uint16_t remainingSpace = EEPROM_PAGE_SIZE - pageOffset;
        const uint16_t toWrite = size - bytesWritten < remainingSpace ? size - bytesWritten : remainingSpace;
        // 使用HAL_I2C_Mem_Write写入数据
        const HAL_StatusTypeDef status = HAL_I2C_Mem_Write(eeprom_hi2c, EEPROM_I2C_ADDR, memAddr,
                                                           I2C_MEMADD_SIZE_16BIT, &data[bytesWritten], toWrite, 100);
...
void at24c32_example() {
    ...
    // 写入数据
    if (EEPROM_Write(eepromAddr, writeData, sizeof(writeData)) != HAL_OK) {
        // 写入失败处理（例如重试或报错）
        printf("EEPROM Write Error\n");
    } else {
        printf("EEPROM Write Success\n");
    }
    if (EEPROM_Read(eepromAddr, readData, sizeof(readData)) != HAL_OK) {
